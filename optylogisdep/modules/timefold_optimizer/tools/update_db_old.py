...
def create_records_from_dataframe(df, model, foreign_key_df_field=None, foreign_model=None, foreign_key_lookup=None):
    for index, row in df.iterrows():
        data = row.to_dict()

        if foreign_key_df_field and foreign_model and foreign_key_lookup:
            ...
            try:
                ...
                data[foreign_model.__name__.lower()] = foreign_obj  # Assigning the foreign key object to the appropriate field
                del data[foreign_key_df_field]  # Remove the foreign key field to prevent errors
...
def df_to_django_model(df: pd.DataFrame, DjangoModel: Type[Model], foreign_key_df_field: str = None,
    ...
    with transaction.atomic():
        ...
        create_records_from_dataframe(df, DjangoModel, foreign_key_df_field=foreign_key_df_field,
...
if __name__ == '__main__':
    ...
    df_to_django_model(df_pers_gr, Pers_gr, foreign_key_df_field='l_pesel', foreign_model=Pers_st,
    ...
    df_to_django_model(df_ind4_om, Ind4_om, foreign_key_df_field='om_id', foreign_model=Osrodek_met,
